...
#define LOG(x) std::ostream(&null_buffer)
#else
...
namespace csvlint {
    ...
    Text::Text (string const &path, size_t max) {
    ...
    int detect_and_trim_eol (Text *lines) {
        ...
        LOG(info) << "detected EOL: " << (unix_ ? "UNIX" : "DOS") << '.';
        return unix_ ? EOL_UNIX : EOL_DOS;
    }
...
not_numeric:
    ...
    void Format::train (string *buffer) {
        Text lines(buffer);
        train(lines);
    }
    ...
    void Format::train (Text &lines) {
        ...
        start_line = prelude;
        if (has_header) ++start_line;
        data_offset = lines[start_line].begin() - lines.origin();
    }
